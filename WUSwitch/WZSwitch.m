...
+(instancetype)createSwitchWithTitle:(NSArray<NSString *> *)array withFrame:(CGRect)frame{
    if (array==nil || array.count<2) {
        NSLog(@"标题内容不符合配置");
        return nil ;
    }
    WZSwitch * switchView = [[WZSwitch alloc]initWithFrame:frame];
    if (array) {
        switchView.leftString = [NSString stringWithFormat:@"%@",[array objectAtIndex:0]];
        switchView.rightString = [NSString stringWithFormat:@"%@",[array objectAtIndex:1]];
    }
    return switchView;
}
